...
def getBestLamdaAndLr(lamdaRange,lrRange):
    ...
    for lr in lrRange:
        for lamda in lamdaRange:
            counts=0.0
            w, b = Logistic_Regression_via_GD(X_train_scaled_for_validation, y_train_cutoff, lr)
            for sample,label in zip(X_val_scaled,y_val):
                if(predict(sample[1:],w,b)==label):
                    counts+=1
...
counts=0
w, b = Logistic_Regression_via_GD(X_train_scaled, y_train, bestlr, bestlamda)
maxacc_this=0
for samepl,label in zip(X_test_scaled,y_test):
    if(predict(sample[1:],w,b)==label):
        counts+=1
maxacc_this=counts/len(y_test)
print(f'best lr found {bestfoundlr} with best lamda: {bestlamda} with accruacy: {maxacc_this}')